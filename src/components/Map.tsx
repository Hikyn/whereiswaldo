...
const Map: React.FC<Props> = ({
  ...
  validateAnswer = (name, posX, posY) => {
    ...
    targets.forEach((target) => {
      if (
        posX > target["endX"] + allowedInaccuracy ||
      ...
      if (
        posY > target["endY"] + allowedInaccuracy ||
  ...
  return (
    <div className="Map-container" onScroll={handleScroll}>
      ...
      <img
        className="map"
        src={map}
        alt="Giant poster with a lot of crossovers"
        onClick={handleClick}
      ></img>
      <footer>
        <div className="card">
          Made by <a href="https://github.com/Hikyn">Hikyn</a>
        </div>
        <div className="card">
          Image by{" "}
          <a href="https://www.instagram.com/ad.2.222/">Egor Klyvchnyk</a>
        </div>
      </footer>
      {isClicked ? (
        <>
