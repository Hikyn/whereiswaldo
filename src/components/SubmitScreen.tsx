...
const SubmitScreen: React.FC<Props> = ({
  finalTime,
  writeScoreToDB,
  isScoreSubmited,
  setIsScoreSubmited,
  ...
  return (
    <div className="SubmitScreen">
      ...
      <h3>
        Final time is: {Math.floor(finalTime / 60)}:
        {finalTime % 60 < 10 ? "0" + (finalTime % 60) : finalTime % 60}
      </h3>
      {!isScoreSubmited ? (
        <form className="submit" onSubmit={handleSubmit}>
          ...
          <div>
            You will need to input secret code in order to submit result.
          </div>
          <div>
            It was made that way to prevent profanity/joke names in results so I
            can sleep soundly knowing that my portfolio piece is safe.
          </div>
        </form>
      ) : (
        <>
          <p>
            If your code was correct, results will show on leaderboard soon!
          </p>
